 ...
 *
 * Every run also regenerates:
 *   xml/             one .drawio per card, the raw <mxGraphModel>, via export-xml.js,
 *                    so each diagram can be opened in draw.io on its own
 *   wiki-bundle.js   every card's source in one file, as WIKI_BUNDLE_SRC. wiki.html
 *                    loads it statically and falls back to it when it cannot fetch a
 *                    card file at runtime — which is what happens inside HTML viewers
 *                    that render the page from a string (Obsidian's Style HTML Viewer
 *                    resolves the <script src> tags it can see, then shows the page in
 *                    a srcdoc iframe where a relative path has nowhere to resolve to)
 *
...
/* path -> its JS source, as the body of an object literal. Shared by the bundle
 * and the --inline build so the two can never disagree about what a card holds. */
function sourcesLiteral(files) {
  return files.map(f =>
...
function renderBundle(files) {
  return [
    '/* wiki-bundle.js — GENERATED by build.js. Do not edit; edit the card and rerun.',
    ' *',
    ' * Every card file\'s source, keyed by its path. wiki.html prefers fetching the',
    ' * real file (so a saved edit shows on refresh in a browser), and uses this copy',
    ' * only when that fetch fails — inside Obsidian\'s HTML viewers, for instance,',
    ' * which resolve the static <script src> tags but cannot fetch at runtime.',
    ' * So: a card edit is live in Chrome at once, and live in Obsidian after the',
    ' * next  node build.js .',
    ' */',
    'WIKI_BUNDLE_SRC = {',
    sourcesLiteral(files),
    '};',
    ''
  ].join('\n');
}
...
function inline(files) {
  ...
  const BUNDLE_TAG = '<script src="wiki-bundle.js">\x3c/script>';
  if (wiki.indexOf(VIEWER_TAG) === -1 || wiki.indexOf(FILES_TAG) === -1 || wiki.indexOf(BUNDLE_TAG) === -1) {
  ...
  // The bundle tag is dropped: WIKI_INLINE_SRC already carries every source.
...
/* Every card's source in one statically loadable file — see the header comment. */
const bundlePath = path.join(ROOT, 'wiki-bundle.js');
const nextBundle = renderBundle(files);
let currentBundle = '';
try { currentBundle = fs.readFileSync(bundlePath, 'utf8'); } catch (e) {}
if (currentBundle === nextBundle) {
  console.log('\nwiki-bundle.js already current.');
} else {
  fs.writeFileSync(bundlePath, nextBundle);
  console.log('\nwiki-bundle.js written — ' + (nextBundle.length / 1024).toFixed(0) + ' KB, ' + files.length + ' file(s) bundled.');
}
